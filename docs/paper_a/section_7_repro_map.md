...
Rows marked **retracted** are retained as artifact provenance only.
They were measured before commit `1646e11` fixed the multi-runner
dynamic-obstacle freeze bug and should not be used as paper-grade
planner evidence.
...
| §5.3 N+P warmup temperature rule | `examples/exp_intersection_{v1,wave,chokepoint}_noisy30_{t01,t10,t100,warmup_select}_mppi_n20.yaml`, `examples/exp_multi_drone_3d_4_noisy05_{t01,t10,t100,warmup_select}_mppi_n20.yaml`, `examples/exp_multi_drone_peer_noisy05_{t01,t10,t100,warmup_select}_mppi_n20.yaml`, `scripts/n_rule_summary.py`, `scripts/warmup_select_validate.py` | "Q: 5-cell N+P rule summary figure" |
| §5.4 planner-family negative for N+P | `examples/exp_{intersection_v1,intersection_wave,intersection_chokepoint,multi_drone_3d_4,multi_drone_peer,city_v1,city_wave,city_chokepoint,city_3x3}_*_n20.yaml`, `scripts/x_planner_family_gather.py`, `docs/data/x_planner_family_data.json`, `docs/images/x_planner_family_landscape.png` | "X: Planner-family landscape — MPC is dominated 0/9 in the dynamic-obstacle regime" |
...
| §6 N+P chokepoint scope condition | `examples/exp_city_chokepoint_noisy30_{mpc,t01_mppi,t10_mppi,t100_mppi,warmup_select_mppi}_n20.yaml`, `examples/exp_intersection_chokepoint_noisy30_{mpc,t01_mppi,t10_mppi,t100_mppi,warmup_select_mppi}_n20.yaml`, `scripts/u_chokepoint_timeseries.py`, `scripts/u_chokepoint_geometry.py`, `docs/data/u_chokepoint_aggregators.json` | "U: chokepoint mechanism — N+P's scope condition is geometric, not signal-based" |
| **retracted** §6 dummy_3d N=4 + moving obstacle speed sweep | `examples/exp_multi_drone_3d_4_dyn_v{2,4,8}{,_gpu_mppi}.yaml`, `scripts/paired_analysis_dummy_3d_multi.py` | pre-`1646e11` artifact provenance only |
| **retracted** §6 dyn off-corridor probe + 2-obstacle compound probe | `examples/exp_multi_drone_3d_4_dyn_{off_v4,2x_v4}{,_gpu_mppi}.yaml` | pre-`1646e11` artifact provenance only |
| **retracted** §6 dyn off-corridor gradient (1, 2, 3 m offsets) | `examples/exp_multi_drone_3d_4_dyn_off{1,2,3}_v4{,_gpu_mppi}.yaml` | pre-`1646e11` artifact provenance only |
| **retracted** Smart MPPI: argmin-fallback option + dynamic paired tests | `uav_nav_lab/planner/gpu_mppi.py` (`fallback_to_argmin` option), `examples/exp_multi_drone_3d_4_dyn_{v2,v4,off2_v4}_gpu_mppi_smart.yaml` | implementation remains; dynamic numbers are artifact provenance only |
| **retracted** Smart MPPI v2: asymmetric perturbation dynamic tests | `uav_nav_lab/planner/gpu_mppi.py` (`asymmetric_bias` option), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_asym.yaml` | implementation remains; dynamic numbers are artifact provenance only |
| **retracted** Smart MPPI v3: temporally-coherent argmin commit dynamic tests | `uav_nav_lab/planner/gpu_mppi.py` (`fallback_commit_steps` option), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_smart_v3.yaml` | implementation remains; dynamic numbers are artifact provenance only |
| **retracted** Smart MPPI v4: mode-aware sampling dynamic tests | `uav_nav_lab/planner/gpu_mppi.py` (`mode_aware_sampling` option), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_smart_v4.yaml` | implementation remains; dynamic numbers are artifact provenance only |
| **retracted** Smart MPPI v5: mode-aware switcher dynamic tests | `uav_nav_lab/planner/gpu_mppi.py` (`mode_aware_lateral_threshold` / `mode_aware_lateral_ratio` options), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_smart_v5.yaml`, `examples/exp_race_oval4_gpu_mppi_smart_v5.yaml` | implementation remains; dynamic numbers are artifact provenance only |
| **retracted** §3 mode 2+4 superposition: drone race + bouncing intruder | `uav_nav_lab/scenario/multi_drone_aerobatic.py` (`radius_y` ellipse option), `examples/exp_race_oval4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_asym}.yaml`, `scripts/render_race_gif.py` | pre-`1646e11` artifact provenance only |
| **retracted** §3 mode-2 mirror image: moving-gates race | `examples/exp_race_gates4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml`, `scripts/render_race_gif.py` (`--config`, `--title`, T_pad) | pre-`1646e11` artifact provenance only |
| **retracted** §3 mode 2 × mode 2-mirror superposition: chaos race | `examples/exp_race_chaos_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml` | pre-`1646e11` artifact provenance only |
| **retracted** §3 mode 4 under dynamic obstacles + avoidance harness: dyn4 path-intersecting intruders | `examples/exp_race_dyn4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml` | pre-`1646e11` artifact provenance only |
| Planner Dijkstra cost-to-go cache tolerance (4-5x speedup) | `uav_nav_lab/planner/{gpu_mppi,mpc}.py` (`ctg_cache_tolerance` option) | "Cost-to-go cache tolerance: 4-5x speedup on moving-goal scenarios" |
