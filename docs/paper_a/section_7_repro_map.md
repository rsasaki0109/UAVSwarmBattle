...
| Smart MPPI v4: mode-aware sampling (cluster-then-softmax) + 4 paired tests | `uav_nav_lab/planner/gpu_mppi.py` (`mode_aware_sampling` option), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_smart_v4.yaml` | "Smart MPPI v4 (mode-aware sampling): the first variant that cracks the cancellation regime" |
| Smart MPPI v5: mode-aware switcher (lateral-cancellation gate) + 5 paired tests | `uav_nav_lab/planner/gpu_mppi.py` (`mode_aware_lateral_threshold` / `mode_aware_lateral_ratio` options), `examples/exp_multi_drone_3d_4{,_dyn_v2,_dyn_v4,_dyn_off2_v4}_gpu_mppi_smart_v5.yaml`, `examples/exp_race_oval4_gpu_mppi_smart_v5.yaml` | "Smart MPPI v5 (mode-aware switcher): lateral-cancellation gate dominates v4 on 4/5 cells" |
| §3 mode 2+4 superposition: drone race + bouncing intruder (n=30 paired, 3-way GIF) | `uav_nav_lab/scenario/multi_drone_aerobatic.py` (`radius_y` ellipse option), `examples/exp_race_oval4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_asym}.yaml`, `scripts/render_race_gif.py` | "Drone race + bouncing intruder: Smart MPPI v4 recovers MPC-level safety without losing tracking precision" |
| §3 mode-2 mirror image: moving-gates race (4 paired sliding gates, n=30 paired, 4-way GIF) | `examples/exp_race_gates4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml`, `scripts/render_race_gif.py` (`--config`, `--title`, T_pad) | "Moving-gates race: the mirror image — softmax wins where it lost the single-intruder race" |
| §3 mode 2 × mode 2-mirror superposition: chaos race (gates + 2 bouncing intruders, n=30) | `examples/exp_race_chaos_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml` | "Drone race chaos — gates + intruders piled on, gate topology still dominates" |
| §3 mode 4 under dynamic obstacles + avoidance harness: dyn4 path-intersecting intruders (n=30, 4-way GIF) | `examples/exp_race_dyn4_{mpc,gpu_mppi,gpu_mppi_smart_v4,gpu_mppi_smart_v5}.yaml` | "dyn4 path-intersecting intruders: controlled dynamic-avoidance harness" |
