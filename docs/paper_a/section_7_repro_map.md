...
| §3 mode 2+4 superposition: drone race + bouncing intruder (3-way GIF) | `uav_nav_lab/scenario/multi_drone_aerobatic.py` (`radius_y` ellipse option), `examples/exp_race_oval4_{mpc,gpu_mppi,gpu_mppi_smart_v4}.yaml`, `scripts/render_race_gif.py` | "Drone race + bouncing intruder: Smart MPPI v4 recovers MPC-level safety without losing tracking precision" |
