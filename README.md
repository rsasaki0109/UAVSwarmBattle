...
<img src="docs/images/intersection_predictor_sweep.png" width="900"><br>
<i><b>E5 predictor-fidelity sweep</b>: replace the perfect
constant-velocity predictor with <code>noisy_velocity</code> at
σ ∈ {0.2, 0.5, 1.0, 3.0, 10.0} or with <code>kalman_velocity</code>.
v1 is binary — predictor <i>presence</i> is a switch, fidelity within
σ ≤ 1 is irrelevant. The harder wave cell reveals a fidelity gradient
at σ ≥ 3: MPPI's softmax holds <b>4/5</b> against MPC's argmin
<b>1/5</b> (averaging across the rollout cloud is robust to bad
predictions), but at σ=10 the order flips (MPC 2/5 vs MPPI 0/5) —
softmax now amplifies a phantom-induced wrong evasion direction with
high enough confidence to commit before recovery. Mechanism plot in
<code>docs/images/intersection_crossover_mechanism.png</code>;
sweep generated by <code>scripts/intersection_predictor_sweep.py</code>.</i>

<br>

The refined §3 framing (see <a href="docs/findings.md">findings.md</a>
for the full E1-E5 chain):
<ul>
<li><b>Success-axis switch</b>: predictor on/off (universal, both cells, both planners drop to 0/5).</li>
<li><b>Success-axis gradient</b>: predictor σ vs intruder pressure (geometry-dependent; emerges on wave at σ ≥ 3, with an aggregator <i>reversal</i> at σ = 10).</li>
<li><b>Fingerprint axis</b>: planner aggregator (argmin → MPC stops & detours wide; softmax → MPPI weaves with smaller |Δcmd|).</li>
</ul>
