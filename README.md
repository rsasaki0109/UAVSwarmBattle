...
<i><b>J: aggregator-temperature sweep (wave cell, n=20)</b>. MPPI's
softmax temperature swept from argmin (t = 0.1) through default
(t = 1.0) to nearly-uniform (t = 10.0). Vanilla MPPI is the <b>worst</b>
aggregator at the σ = 3 knee (35%); both extremes recover. Argmin MPPI
(t = 0.1) wins at <b>70%</b>, beating MPC's 45% by 25 percentage points.
The "valley of bad" sits exactly at the default t = 1.0 — softmax
averages similar-cost rollouts into a phantom-evasion direction with
just enough confidence to commit but not enough to argmin out of it.
Generated by <code>scripts/intersection_temperature_sweep.py</code>.</i>

<br>

The refined §3 framing (see <a href="docs/findings.md">findings.md</a>
for the full E1-J chain):
<ul>
...
<li><b>Fingerprint axis</b>: planner aggregator. MPPI's aggregator-temperature is a U-shape — vanilla (t = 1.0) is the worst aggregator at the knee, argmin MPPI is the most robust choice when predictions are noisy but informative.</li>
</ul>
