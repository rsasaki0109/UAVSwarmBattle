...
- **The convention survives real AirSim flight physics — the whole arc is not a point-mass artefact.** Every result above ran on the kinematic `dummy_3d` sim (velocity-integrated points, frictionless "veer right"). Lift the N=4 antipodal hub into [AirSim](https://github.com/microsoft/AirSim)'s Blocks Unreal physics — SimpleFlight quadrotors with gravity, momentum, real mesh-to-mesh collision, the synchronised clock — same MPC, same `lateral_bias` knob, CV peer prediction **on** in both arms: stock collides on 9 of 16 seeds (joint **43.8 %**, the antipodal `XXXX`/`SXSX` signature), the convention clears **every drone, every seed** (joint **100 %**; McNemar b=0, c=9, p=3.9e-3). The ~10-line decentralised rule validated only on points transfers intact to quadrotor flight (<code>scripts/run_airsim_antipodal_chunked.sh</code>; <a href="docs/findings.md#the-right-of-way-convention-breaks-the-antipodal-deadlock-under-real-airsim-flight-physics-not-just-the-kinematic-sim">real-physics reproduction</a>).
...
- **A K-way flocking hub jams at every fan-in; the roundabout clears it — but structure goes before safety.** Place K flocks on a ring all migrating through the centre (the flocking analogue of the N-drone antipodal hub). Without a convention the hub **gridlocks totally at every K** (all-passed 0/40); the right-of-way veer turns the fan-in into a **roundabout** and clears it at every K=2…12 (40/40, c=40, p=1.8e-12) — where the point-agent hub has a density cliff, the flocking roundabout *scales*. But the costs onset in order: a *jammed* hub keeps each flock intact (cohesion ≈1.0), while the roundabout that frees them **shears** them — cohesion falls 0.86→0.75 as the hub crowds, then plateaus (it costs ~20–25 % of each flock to thread a contested centre). The safety margin is the **last** casualty: inter-flock spacing holds through K=10 (0/20 collide), then the hub saturates — too many agents through one point — and the collision cliff finally appears (K=12: 5/20, K=14: 18/20). Crowding a flocking hub costs **structure before safety**.
- **For a flock, who follows the rule matters more than how many — cohesion makes adoption a placement problem.** Hand the right-of-way veer to only *some* agents in the K-way hub. Within a flock a **minority suffices**: just **30 % of each flock** following the rule clears the hub fully (18/60 → 40/40, p=1.8e-12; even 20 % rescues 10/40) because the α-lattice (velocity consensus) **drags** the free-riders around the roundabout — where the [point-agent convention](docs/findings.md#the-right-of-way-convention-needs-near-full-adoption--free-riders-break-it-and-tolerance-shrinks-with-density) needed near-unanimity. And at a *fixed* adoption budget, **placement beats count**: spread thin across every flock (MIXED) vs clumped into whole flocks (CLUSTERED), MIXED wins at every budget (15/60: 28/40 vs 14/40, b=1 c=15, p=5e-4; 18/60: 40/40 vs 30/40, p=2e-3). Clump the rule into a few flocks and the un-adopting flocks stay **coherent walls** that re-jam the hub — the flocking echo of the point-agent non-cooperator pair. The cost flips too: CLUSTERED keeps slightly higher cohesion (intact free-rider flocks) but jams more. A flock's convention is *placement-sensitive, not head-count-sensitive*.
...
<br><sub><b>The convention, validated in real flight physics.</b> The N=4 antipodal hub flown in <a href="https://github.com/microsoft/AirSim">AirSim</a> (quadrotor dynamics, real collision), top-down x-y, same MPC. <b>Stock</b> (left) drives all four into the centre and they <b>collide</b> (red ring); the <b>right-of-way bias</b> (right) makes them <b>veer and clear</b> to their antipodal goals. Over 16 seeds the deadlock is real even with CV prediction (stock joint 43.8 %) and the convention fixes it completely (100 %, McNemar p=3.9e-3) — the whole right-of-way arc is <b>not</b> an artefact of the kinematic point model (<code>scripts/render_airsim_antipodal_gif.py</code>; <a href="docs/findings.md#the-right-of-way-convention-breaks-the-antipodal-deadlock-under-real-airsim-flight-physics-not-just-the-kinematic-sim">the result</a>).</sub>
</div>
...
<br><sub><b>A K-way flocking hub: jam vs roundabout.</b> Six flocks (distinct colours) migrate through a shared centre. <b>No convention</b> (left) they <b>JAM</b> at the hub — a mutual wall of α-repulsion. <b>Right-of-way</b> (right) — each agent veers right of its goal — and the fan-in becomes a <b>ROUNDABOUT</b> that clears every flock. The jam is total at every fan-in (all-passed 0/40) and the roundabout clears it at every K=2…12 (40/40, p=1.8e-12) — but it <b>shears</b> the flocks (cohesion 0.86→0.75) and, at the densest hubs, finally collides (K=12: 5/20, K=14: 18/20). Crowding a flocking hub costs <b>structure before safety</b> (<code>scripts/render_hub_gif.py</code>; <a href="docs/findings.md#a-k-way-flocking-hub-jams-at-every-fan-in-the-roundabout-convention-clears-it--cohesion-is-the-first-casualty-collisions-only-the-last">the hub</a>).</sub>
</div>
...
<br><sub><b>Placement beats head-count.</b> The <b>same</b> adoption budget (15/60 follow the right-of-way veer; adopters wear a black ring, free-riders are pale). <b>CLUSTERED</b> (left) — whole flocks adopt, the rest free-ride entirely — and the un-adopting flocks stay <b>coherent walls</b> that jam the hub. <b>MIXED</b> (right) — the same followers spread thin across every flock — and each flock's cohesion <b>drags its free-riders through</b> the roundabout. MIXED wins at every budget (15/60: 28/40 vs 14/40, p=5e-4) and a mere 30 % of each flock clears the hub fully. A flock's convention is <b>placement-sensitive, not head-count-sensitive</b> (<code>scripts/render_critmass_gif.py</code>; <a href="docs/findings.md#who-must-follow-the-roundabout-rule-a-flocks-cohesion-makes-adoption-a-placement-problem-not-a-head-count">who must adopt</a>).</sub>
</div>
