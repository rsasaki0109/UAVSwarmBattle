...
Output: docs/images/intersection_crossover_mechanism_n20.png — 2×2:
...
OUT = Path("docs/images/intersection_crossover_mechanism_n20.png")
...
def _plot_vy_pane(ax, mpc_dir, mppi_dir):
    ...
    for planner, run_dir in [("MPC", mpc_dir), ("MPPI", mppi_dir)]:
        ...
        outs = [_load(run_dir, ep)[0]["outcome"] for ep in range(20)]
        n_col = sum(1 for o in outs if o == "collision")
        ax.plot([], [], "-", color=PLANNER_COLORS[planner], lw=2.0,
                label=f"{planner}: {n_col}/20 drone-north collisions")
...
def main() -> int:
    ...
    _plot_traj_pane(
        ax_a,
        "results/intersection_wave_noisy100_mpc_n20",
        "results/intersection_wave_noisy100_mppi_n20",
        WAVE_YAML, ep=0,
        title="(a) noisy σ=10 ep 0 (n=20, seeded) — both planners panic and reverse "
              "near the wave; MPPI commits earlier and dies (X)")
    _plot_traj_pane(
        ax_b,
        "results/intersection_wave_noisy30_mpc_n20",
        "results/intersection_wave_noisy30_mppi_n20",
        WAVE_YAML, ep=0,
        title="(b) noisy σ=3 ep 0 — both planners struggle at the knee "
              "(n=20: MPC 9/20 = 45%, MPPI 7/20 = 35%); timing fingerprint visible")
    _plot_vy_pane(
        ax_c,
        "results/intersection_wave_noisy100_mpc_n20",
        "results/intersection_wave_noisy100_mppi_n20")
    ...
    fig.suptitle(
        "σ=10 chaos regime (n=20, seeded predictor) — both planners U-turn into "
        "the wave; MPPI's softmax commits ~250 ms earlier (timing fingerprint "
        "survives), but no crossover — both planners collapse to ~10% (noise floor)",
        fontsize=10, y=0.998,
    )
